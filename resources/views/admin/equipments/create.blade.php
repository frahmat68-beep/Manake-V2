...
<div class="max-w-3xl mx-auto space-y-8 font-sans">
    ...
    <div class="border-b border-zinc-800 pb-6">
        ...
        <h1 class="text-3xl font-black text-amber-500 tracking-wider uppercase">TAMBAH PERALATAN</h1>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input 
                        ...
                        placeholder="dji-ronin-sc (otomatis jika dikosongkan)"
                    >
                    @error('slug')
                        <p class="text-xs text-red-500 font-light mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="space-y-2">
                    ...
                    <select 
                        ...
                        class="w-full bg-zinc-950 border border-zinc-850 focus:border-amber-500 rounded-lg text-sm px-4 py-3 text-zinc-100 focus:ring-1 focus:ring-amber-500 transition-all outline-none"
                    >
                        <option value="ready" {{ old('status') == 'ready' ? 'selected' : '' }}>Tersedia (Ready)</option>
                        <option value="maintenance" {{ old('status') == 'maintenance' ? 'selected' : '' }}>Pemeliharaan (Maintenance)</option>
                        <option value="unavailable" {{ old('status') == 'unavailable' ? 'selected' : '' }}>Tidak Siap (Unavailable)</option>
                    </select>
                    @error('status')
                        <p class="text-xs text-red-500 font-light mt-1">{{ $message }}</p>
                    @enderror
